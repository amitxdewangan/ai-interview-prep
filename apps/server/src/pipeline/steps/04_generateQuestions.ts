...
type CategoryQuestionDraft = z.infer<typeof CategoryQuestionsOutputSchema>[number];

/**
 * Step 3: Deliberate multi-step question generator.
...
Return a JSON array matching the schema.`;
  ...
  try {
    const techQuestions = await llm.generateStructured(
      techPrompt,
    ...
    allDrafts.push(...techQuestions);
  } catch {
    // If call fails, synthesize minimal questions from requirements
    allDrafts.push(...fallbackQuestionsForReqs(techTargets, 'technical'));
  }
...
- "category": strictly "behavioural" or "company-fit"
...
Return a JSON array matching the schema.`;
  ...
  try {
    const behavQuestions = await llm.generateStructured(
      behavPrompt,
      CategoryQuestionsOutputSchema,
      {
        systemInstruction:
    ...
    allDrafts.push(...behavQuestions);
  } catch {
    allDrafts.push(...fallbackQuestionsForReqs(behavTargets, 'behavioural'));
  }
...
For each question:
- "requirement_ids": array of linked requirement IDs
- "category": strictly "system-design" or "company-fit"
- "prompt": system design scenario or domain architectural challenge
...
Return a JSON array matching the schema.`;
  ...
  try {
    const designQuestions = await llm.generateStructured(
      designPrompt,
      CategoryQuestionsOutputSchema,
      {
        systemInstruction:
          'You are a systems design interviewer. Generate realistic architecture and domain design challenges linked to requirement IDs.',
        temperature: 0.2,
      }
    );
    allDrafts.push(...designQuestions);
  } catch {
    allDrafts.push(...fallbackQuestionsForReqs(domainTargets, 'system-design'));
  }
